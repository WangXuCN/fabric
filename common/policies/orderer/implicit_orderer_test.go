...
func TestPolicyProvider_NewPolicy(t *testing.T) {
	...
	t.Run("good", func(t *testing.T) {
		policyBytes, _ := proto.Marshal(&cb.ImplicitOrdererPolicy{Rule: cb.ImplicitOrdererPolicy_SMARTBFT})
...
func TestImplicitBFTPolicy_Evaluate(t *testing.T) {
	...
	sig3 := []byte{3, 8, 9}
	sig4 := []byte{4, 8, 9}
	sd1 := &protoutil.SignedData{Data: data, Identity: consenters[0].Identity, Signature: sig1}
	sd2 := &protoutil.SignedData{Data: data, Identity: consenters[1].Identity, Signature: sig2}
	sd3 := &protoutil.SignedData{Data: data, Identity: consenters[2].Identity, Signature: sig3}
	sd4 := &protoutil.SignedData{Data: data, Identity: consenters[3].Identity, Signature: sig4}
	...
	t.Run("3 good sigs, 3 out of 4", func(t *testing.T) {
		...
		signatureSet := []*protoutil.SignedData{sd1, sd2, sd3}
	...
	t.Run("3 good sigs 1 bad, 3 out of 4", func(t *testing.T) {
		...
		signatureSet := []*protoutil.SignedData{sd1, sd2, sd3, sd4}
		err = p.EvaluateSignedData(signatureSet)
		assert.NoError(t, err)
	})
